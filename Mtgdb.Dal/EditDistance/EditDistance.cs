...
namespace Mtgdb.Dal
{
	public class EditDistance
	{
		...
		public const int MaxInput = 20;
		private readonly float[,] _trace = new float[MaxInput + 1, MaxInput + 1];
		...
		public float GetPrefixDistance(string userStr, string dictStr)
		{
			...
			if (userStr.Length > MaxInput)
				throw new ArgumentException($"user string length exceeds {MaxInput}");

			if (dictStr.Length > MaxInput)
				throw new ArgumentException($"dict string length exceeds {MaxInput}");
			...
			var maxDictLength = 2 + userLength*2;
			if (dictLength > maxDictLength)
				dictLength = maxDictLength;
