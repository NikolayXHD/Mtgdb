	...
	public static class LayoutViewControlScaler
	{
		...
		private static
			DpiScaler<LayoutViewControl,
				...
				(Bitmap, Bitmap, Bitmap),
				(Bitmap, Bitmap))> createViewOptionsScaler(Func<Bitmap, Bitmap> transformIcons)
			{
				return Scalers.Combine(
					Scalers.Combine(
						new DpiScaler<LayoutViewControl, Dictionary<Direction, Bitmap>>(
							c => c.LayoutOptions.AlignmentIconsByDirection,
							(c, d) =>
							{
								foreach (var pair in d)
									c.LayoutOptions.AlignmentIconsByDirection[pair.Key] = pair.Value;
							},
							d => d.ToDictionary(_ => _.Key, _ => _.Value?.Invoke0(transformIcons))
						),

						new DpiScaler<LayoutViewControl, Dictionary<Direction, Bitmap>>(
							c => c.LayoutOptions.AlignmentHoveredIconsByDirection,
							(c, d) =>
							{
								foreach (var pair in d)
									c.LayoutOptions.AlignmentHoveredIconsByDirection[pair.Key] = pair.Value;
							},
							d => d.ToDictionary(_ => _.Key, _ => _.Value?.Invoke0(transformIcons))
						)),
